...
```bash
aws-lambda-powertools-typescript$ sam build
```
...
```bash
aws-lambda-powertools-typescript$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
aws-lambda-powertools-typescript$ sam local start-api
aws-lambda-powertools-typescript$ curl http://localhost:3000/
```
...
```bash
aws-lambda-powertools-typescript$ sam logs -n HelloWorldFunction --stack-name aws-lambda-powertools-typescript --tail
```
...
```bash
aws-lambda-powertools-typescript$ cd hello-world
...
```bash
sam delete --stack-name aws-lambda-powertools-typescript
```
